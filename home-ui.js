(function(){
  function boot(){
    ...
    /* Plateau : on pousse le rendu vers un vrai Mahjong mobile pirate, sans toucher aux règles de jeu. */
    if(!document.getElementById('pirateBoardPolish')){
      ...
      .board{border-width:5px!important;border-color:#a96b2c!important;border-radius:22px!important;background:
        radial-gradient(circle at 50% 48%,#2a9493 0,#11737e 43%,#075363 72%,#043746 100%),
        repeating-linear-gradient(168deg,transparent 0 24px,#ffffff08 25px 26px)!important;
        box-shadow:inset 0 0 0 2px #e0b75a55,inset 0 0 28px #001f2b,inset 0 -18px 28px #00192388,0 7px 0 #4a2513,0 14px 26px #00131dcc!important}
      .board .tile{border-width:2px!important;border-radius:9px!important;background-clip:padding-box;letter-spacing:0;line-height:1;text-shadow:0 1px 0 #ffffff99;box-shadow:
        inset 2px 2px 0 #fffdf2,
        inset -2px -3px 0 #8c6b3f,
        2px 3px 0 #51351d,
        0 5px 9px #00151aaa!important;
      ...
      .board .tile.selected{outline:3px solid #ffe07a;outline-offset:2px;box-shadow:inset 2px 2px 0 #fff,inset -2px -3px 0 #8c6b3f,4px 7px 0 #51351d,0 0 20px #ffd95d!important;z-index:99!important}
      ...
      .board .tile.selected{animation:pirateBoardSelect .55s ease-in-out infinite alternate!important}
      @keyframes pirateBoardSelect{from{transform:translateY(-5px) scale(1.035)}to{transform:translateY(-8px) scale(1.07)}}
      .boosters{margin-top:12px;padding-top:10px;position:relative}
      .boosters:before{content:'☠  OUTILS DU CAPITAINE  ☠';position:absolute;top:-3px;left:50%;transform:translateX(-50%);font-size:.55rem;letter-spacing:.13em;color:#d7b15c;white-space:nowrap;opacity:.78}
      @media(max-width:600px){.board{width:342px!important;height:588px!important;border-radius:20px!important}.board .tile{width:52px!important;height:62px!important;font-size:1.55rem!important}.boosters:before{font-size:.48rem}}
