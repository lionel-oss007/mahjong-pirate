...
// Règle Mahjong Solitaire : une tuile est libre si elle n'est recouverte par aucune
// tuile supérieure et si au moins un de ses deux côtés horizontaux est ouvert.
...
// Formes pirates : chaque niveau garde exactement 24/28/32/36/40 tuiles.
// Les formes restent compactes pour conserver une vraie sensation de plateau.
function layout(level){
  ...
  const rows=shapes[level][0],positions=[];
  ...
  if(level>=3){
    const centerRows=level===3?[2,3]:level===4?[2,3,4]:[2,3,4];
    let lifted=0;
    centerRows.forEach(r=>{for(const p of positions.filter(p=>Math.round((p.y-25)/STEP_Y)===r).slice(1,-1)){if(lifted<4){p.l=1;p.x+=11;p.y+=10;lifted++}}});
  }
  return positions.map((p,i)=>({...p,id:i}));
}

// On simule des retraits légaux puis on donne le même symbole à chaque paire.
// Ainsi le plateau initial possède toujours une solution connue.
function buildSolvable(level){
  for(let attempt=0;attempt<500;attempt++){
    ...
    while(remaining.some(t=>!t.removed)){
      ...
      const a=free[0], candidates=free.filter(t=>t.id!==a.id);
      if(!candidates.length)break;
      const b=candidates[Math.floor(Math.random()*candidates.length)];
      a.removed=b.removed=true;pairs.push([a.id,b.id]);
    }
    if(pairs.length===counts[level]/2){
  ...
  // Secours : toutes les tuiles sont sur une seule ligne logique et donc libres.
  const positions=[];for(let i=0;i<counts[level];i++)positions.push({id:i,x:45+(i%8)*STEP_X,y:25+Math.floor(i/8)*STEP_Y,l:0,removed:false,symbol:symbols[Math.floor(i/2)%symbols.length]});
  return positions;
}

function newGame(level){clearInterval(timer);game={level,tiles:buildSolvable(level),selected:null,score:0,moves:0,pairs:0};startedAt=Date.now();$('map').classList.add('hidden');$('game').classList.remove('hidden');$('islandTitle').textContent=names[level];$('difficulty').textContent=difficulties[level];$('msg').textContent='🀄 Règle : retire uniquement deux symboles identiques et libres.';render();timer=setInterval(tick,500)}
...
function pick(id){const t=game.tiles.find(x=>x.id===id);if(!t||!freeTile(t,game.tiles))return;if(game.selected===null){game.selected=id;render();return}const a=game.tiles.find(x=>x.id===game.selected);game.moves++;if(a&&a.id!==t.id&&a.symbol===t.symbol&&freeTile(a,game.tiles)){a.removed=t.removed=true;game.pairs++;game.score+=100+game.level*15;game.selected=null;sfx(true);$('msg').textContent='⚓ Paire valide ! Les deux tuiles sont retirées.'}else{game.selected=null;sfx(false);$('msg').textContent='❌ Paire impossible : mêmes symboles + deux tuiles libres.'}render();if(game.tiles.every(t=>t.removed))win();else if(!legalPairs().length)$('msg').textContent='☠️ Aucune paire disponible. Utilise Mélanger.'}
